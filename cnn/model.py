...
if __name__ == "__main__":
    ...
    def get_random_config(config, cache):
        generated = False
        res = None
        while not generated:
    ...
    all_tranformations = [t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16]

    folds = range(5)

    loss_func = nn.BCELoss()

    def train_one_configuration(config, l_fn):
        scores = []

        for fold in folds:
            main_logger = Logger("../logs/%s" % fold)
            train_sets = [IcebergDataset("../data/folds/train_%s.npy" % fold, transform=t, top=top)
                          for t in all_tranformations]
            val_ds = IcebergDataset("../data/folds/test_%s.npy" % fold, transform=ToTensor(), top=top)
            model_prefix = str(hash(str(config)))
            net = LeNet(2, config["conv"], 128, 32, fold_number=fold, gain=config["gain"], model_prefix=model_prefix)
            ...
            best = net.fit(optim, l_fn, train_loaders, val_loader, 25, logger=main_logger)
            print()
            print("Best was ", best)
            scores.append(best)
        return scores
    ...
    all_scores = []
    for _ in progressbar(range(65)):
        current_config = get_random_config(parameter_grid, cache)
        current_score = train_one_configuration(current_config, loss_func)
        current_score.append(str(current_config))
